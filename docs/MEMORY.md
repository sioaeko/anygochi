---
name: anygochi project
description: anygochi — CLI 코딩 툴용 다마고치 버디 시스템, 현재 상태 및 미해결 문제
type: project
---
...
| 파일 | 역할 |
|------|------|
| `~/.local/bin/anygochi` | 메인 Python 앱 (show/session/watch/reset/info/list) |
| `~/.local/bin/anygochi-wrap` | 래퍼 생성기 (wrap/--list/--remove) |
| `~/.local/share/anygochi/BuddyPanel.js` | Gemini CLI용 Ink 컴포넌트 |
| `~/.local/share/anygochi/DefaultAppLayout.patched.js` | Gemini CLI 레이아웃 패치 |
| `~/.local/share/anygochi/gemini-patch.sh` | Gemini 패치 sudo 스크립트 |
| `~/.local/share/anygochi/gemini-unpatch.sh` | Gemini 패치 롤백 스크립트 |
...
- `~/.config/anygochi/state.json` — 세션 수, 크리처 종류, 부화 여부
...
- 증상: `opencode` 실행 시 tmux 세션 생성되지만 opencode 패인이 바로 종료, anygochi watch 패인만 남음
...
- `_sess="anygochi-$$"` PID 고정 버그는 수정 완료 (2026-04-01)

**Task #6:** cursor 스크립트에 anygochi 주입 (미시작)
