...
**다른 언어:** [English README.md](README.md)
...
| **런타임** | Python 3.8+ (`rich` 선택), 선택적 `tmux`, 선택적 [tokscale](https://github.com/sioaeko/tokscale) |
...
| **패인 너비** | 우측 `watch` 폭 | `OPENBUDDY_TMUX_PCT` (기본 4%); 런처: `OPENBUDDY_TMUX_COLS` (기본 8칸) |
...
기본적으로 **`show` / `watch`는 컴팩트 레이아웃**(짧은 ASCII, 세로로 덜 씀). 넓은 터미널에서 예전처럼 **풀 패널**(스탯 바·긴 ASCII)을 쓰려면 **`OPENBUDDY_FULL_UI=1`** 또는 `~/.config/openbuddy/config.json`에 **`"full_ui": true`** 를 넣으면 됩니다.
